...
def init_process(core_index, test_path):
    ...
    if dir_existed:
        ...
        op.mkdir("%s/cache" % outcomes_path)
   

'''
Fuzzing:
Run cmd line to generate mutated outputs.
Save outputs as outcome_*/cache/id_index.
Remove duplicates using afl-cmin.
@param: fuzzer_name and other option
'''
def fuzzing_process(core_index, test_path, fuzzer_path, mutation_num, seed_path,program_path):
    outcomes_path = "%s/outcomes-%d" % (test_path, core_index)
    fuzzer_run_cmd = "%s -o %s/cache/id_%s -n %d %s" % (fuzzer_path,outcomes_path, "%n", mutation_num, seed_path)
    fuzzer_return = op.run_cmd(fuzzer_run_cmd)
    
    #remove duplicates using afl-cmin "afl-cmin -i afl_in -o afl_out -m none -- cxxfilt"
    rmdup_cmd = "afl-cmin -i %s/cache -o %s/out -m none -- %s" % (outcomes_path, outcomes_path, program_path)
    print(rmdup_cmd)
    #rmdup_return = op.run_cmd(rmdup_cmd)
    rmdup_pipe = os.popen(rmdup_cmd)
    rmdup_pipe.read()
    rmdup_pipe.close()
...
def run_radamsa_subprocess(core_index, test_path, program_path, fuzzer_path, seed_path, mutation_num):
     ...
     while True:
          ...
          fuzzing_process(core_index, test_path, fuzzer_path, mutation_num, seed_path, program_path)
